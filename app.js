...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <div class="profile-section">
            ...
            <div><h2 style="margin:0; font-size:1rem;">${user.username}</h2><p style="margin:0; font-size:0.7rem; color:#007bff;">${isAdmin ? 'Kurucu Admin' : 'Zweed VIP'}</p></div>
        </div>
        ...
        ${isAdmin ? '<a href="/admin" style="background: #007bff22; color: #007bff;"><i class="fas fa-shield-alt"></i> Admin Paneli</a>' : ''}
...
const checkAdmin = (req, res, next) => { (req.session.user && req.session.user.role === 'admin') ? next() : res.send("Yetkin yok kanka!"); };
...
app.post('/register', (req, res) => {
    ...
    users.push({ ...req.body, avatar: null, role: role });
    saveUsers(users);
    res.redirect('/login');
});
...
app.get('/admin', checkAuth, checkAdmin, (req, res) => {
    ...
    const list = files.map(f => `
        <div style="background:#0b0e11; margin:8px 0; padding:12px; border-radius:12px; border:1px solid #333; display:flex; justify-content:space-between; align-items:center; text-align:left;">
            <span style="font-size:0.7rem; color:#aaa; overflow:hidden;">${f.substring(0,20)}</span>
            <a href="/admin/delete/${f}" class="btn-danger" onclick="return confirm('Siliyom bak?')">SİL</a>
        </div>`).join('');
    res.send(THEME(`<h1>Admin Kontrol</h1><p style="font-size:0.8rem; color: #666;">Tüm dosyalara tam erişim.</p>${list || '<p>Bulut boş.</p>'} <a href="/" style="color:#007bff; text-decoration:none; display:block; margin-top:20px;">Geri Dön</a>`, req.session.user));
});
