...
// --- MODERN MAVİ-SİYAH TEMA & PARLAMA FİX ---
...
<head>
    ...
    <style>
        * { -webkit-tap-highlight-color: transparent !important; outline: none !important; box-shadow: none !important; user-select: none; }
        ...
        h3 { color: #eee; font-size: 1rem; margin-top: 20px; text-align: left; }
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <div class="profile-section">
            ...
            <div><h2 style="margin:0; font-size:1rem;">${user.username}</h2><p style="margin:0; font-size:0.7rem; color:#007bff;">Zweed VIP</p></div>
        </div>
...
app.post('/register', (req, res) => {
    ...
    users.push({ ...req.body, avatar: null });
    saveUsers(users);
    res.redirect('/login');
});

// HESAP PANELİ (İsim, Foto ve Şifre Güncelleme)
app.get('/hesap', checkAuth, (req, res) => {
    res.send(THEME(`
        <h1>Hesap Paneli</h1>
        <form action="/update-account" method="POST" enctype="multipart/form-data">
            <h3>Profil Bilgileri</h3>
            <input name="newUsername" value="${req.session.user.username}" placeholder="Yeni Kullanıcı Adı">
            <input type="file" name="avatar" accept="image/*">
            
            <h3>Güvenlik</h3>
            <input name="newPassword" type="password" placeholder="Yeni Şifre (Boş bırakırsan değişmez)">
            
            <button class="btn">BİLGİLERİ GÜNCELLE</button>
        </form>
        <a href="/" style="color:#007bff; text-decoration:none; display:block; margin-top:20px; font-size:0.9rem;">⬅ Ana Sayfaya Dön</a>
    `, req.session.user));
});
...
app.post('/update-account', checkAuth, multer({dest: AVATARS_DIR}).single('avatar'), (req, res) => {
    ...
    if(req.body.newPassword && req.body.newPassword.trim() !== "") users[idx].password = req.body.newPassword;
    if(req.file) users[idx].avatar = req.file.filename;
    
    saveUsers(users);
    req.session.user = users[idx];
    res.send("<script>alert('Başarıyla güncellendi!'); window.location='/hesap';</script>");
});
...
app.get('/dosyalar', checkAuth, (req, res) => {
    ...
    const list = files.map(f => `<div style="background:#0b0e11; margin:8px 0; padding:12px; border-radius:12px; border:1px solid #333; display:flex; justify-content:space-between; align-items:center;"><a href="/download/${f}" target="_blank" style="color:#007bff; text-decoration:none; font-size:0.8rem; overflow:hidden;">${f.substring(0,25)}</a></div>`).join('');
    res.send(THEME(`<h1>Dosyalarım</h1>${list || '<p style="color:#666;">Dosya bulunamadı.</p>'}<a href="/" style="color:#007bff; text-decoration:none; display:block; margin-top:20px;">Geri Dön</a>`, req.session.user));
});

app.get('/logout', (req, res) => { req.session.destroy(); res.redirect('/login'); });
app.listen(PORT, () => console.log('Zweed Cloud v8 Online!'));
